...
def embed (df_space, df_structures, n_dim):
    ...
    for hc_order in range (2, max_z + 1):
        ...
        print ('Generating HCASE embedding at parameter z (PHC order): %d, nr_scaffolds: %d, bucket_nr: %d,  bucket_size %f ..' % (hc_order, nr_scaffolds, bucket_nr, bucket_size))
        ...
        df_hilbert['embedded_hs_coordinates'] = df_hilbert.apply (lambda x: get_hilbert_coordinates(hilbert_curve, x['bucket_id']), axis = 1)  
